...
// Si el password no es v�lido devuelve el error, si no, no devuelve nada. 

// De momento los criterios para que no sea v�lido son: 
//	- difiere de su validaci�n.
...
function password_valido ($password, $validacion) {
	...
	if (is_numeric($password)) {
		$error = "El password no puede ser num�rico";
		return $error;
	}
	...
	if (strcmp($password,$validacion) <> 0) {
		$error = "El password y su validaci�n no son iguales";
		return $error;
	}
...
//	Lo que hace es revisar todos los par�metros recibidos en el POST
// 		y aquellos que tengan valor los coloca en una cadena lista para PRE

// El primer par�metro ser�n aquellas variables que queremos a�adir a PRE
...
function post_to_pre() {
	...
	while ($key = $keys[$i]) {
		...
		$i += 1;
			...
			// Ahora a�adimos las que queremos
	$i = 0;
	while($queremos[$i]) {
		if ($no_inicio) {
			$return .= "&";
...
function radio_boolean() {
	...
	$select = "	<input type='radio' name='$name' value='0'>No
					&nbsp;
				<input type='radio' name='$name' value='1'>S�
				";
